...
CVAR(Bool, gl_spritedepth, false, CVAR_ARCHIVE)	//[XANE]Include sprites in depth map in exchange for glitchy translucent
												//pixel rendering in the depth map?
...
void HWDrawInfo::RenderTranslucent(FRenderState &state)
{
	...
	state.SetDepthMask(gl_spritedepth); //[XANE]Set this to TRUE and sprites will display in the depth map, though trees
	                                    //don't display correctly (and still no HUD).
