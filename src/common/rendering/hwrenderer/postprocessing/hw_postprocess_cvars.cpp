...
CUSTOM_CVAR(Int, gl_fxaa, 2, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
{
...
CUSTOM_CVAR(Int, gl_ssao, 2, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
{
	if (self < 0 || self > 3)
		self = 0;
}
...
CVAR(Float, gl_ssao_radius, 17.5f, 0)
CUSTOM_CVAR(Float, gl_ssao_blur, 8.0f, 0)
{
	if (self < 0.1f) self = 0.1f;
}

CUSTOM_CVAR(Float, gl_ssao_exponent, 0.5f, 0)
{
	if (self < 0.1f) self = 0.1f;
}
